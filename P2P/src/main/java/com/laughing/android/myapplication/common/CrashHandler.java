...
/**
 * Created by laughing on 2015/12/13.
 * <p/>
 * CrashHandler设计为单例模式：用来处理系统未不活的异常
 */
...
//        Log.e("zoubo", "CrashHandler----uncaughtException....");
    ...
    private void collectionException(Throwable ex) {
        final String deviceInfo = "DEVICE=" + Build.DEVICE + "SDK_INT=" + Build.VERSION.SDK_INT + "MODEL=" + Build.MODEL + "PRODUCT=" + Build.PRODUCT;
        final String errorInfo = ex.getMessage();
        new Thread() {
            @Override
            public void run() {
                //有服务器可以发送到服务器
                Log.e("TAG", "deviceInfo---" + deviceInfo + ":errorInfo" + errorInfo);
            }
        }.start();
    }
}
